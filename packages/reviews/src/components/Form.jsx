...
const STATE_KEYS = ['review', 'reviews', 'member', 'members', 'error', 'status', 'taskService']
...
const create = async (state, dispatch, e) => {
  ...
  const { reviews, choicesText, title, description } = state
  ...
  try {
    await reviews.create({ title, description, choices })
...
const update = async (state, dispatch, e) => {
  e.preventDefault()
  resetAlerts(dispatch)
  const { review, reviews, choicesText, title, description } = state
  if (![title, choicesText, description].reduce((acc, e) => acc && !!e.length, true)) {
    dispatch({ type: 'error', payload: 'all fields are required' })
    return
  }
  ...
  const choices = textToArray(choicesText)
  try {
    await reviews.patch(reviewId, { title, description, choices })
  ...
  useEffect(() => {
    (async () => {
      ...
      if (mode === MODES.update) {
        ...
        dispatch({ type: 'review', payload: entity })
